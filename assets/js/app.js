...
*/
import { CALCULATORS, getCalc, computeCalc, fmtMoney, fmtCompactMoney, fmtPct, fmtNum, clamp } from "./engine.js";
...
function buildForm() {
  ...
  state.defs.forEach((f) => {
    ...
    const setVal = (v) => {
      const n = clamp(Number.isFinite(v) ? v : Number(v), f.min, f.max);
      state.values[f.key] = n;
      input.value = n;
      range.value = n;
    };
    setVal(state.values[f.key] ?? f.def);
    input.addEventListener("input", debounce(() => {
      let n = Number(input.value);
      n = Number.isFinite(n) ? clamp(n, f.min, f.max) : f.min;
      setVal(n);
      userRender();
    }, 60));
    input.addEventListener("blur", () => { setVal(state.values[f.key]); });
    range.addEventListener("input", () => { setVal(Number(range.value)); userRender(); });
    state.calcEls[f.key] = { el: input, def: f, range, setVal };
  });
  mount.innerHTML = "";
...
function render() {
  ...
  const result = computeCalc(state.slug, state.values);
  if (!result) return;
...
function renderHead(head) {
  ...
  if (head.kind === "years") {
    const years = Number(head.value) || 0;
    tag("span", "result-head__years", card, `${fmtNum(years, 2)} years`);
  } else if (head.kind === "money") {
    const val = Number(head.value) || 0;
    if (val < 0 || head.negative) card.classList.add("is-neg");
    tag("span", "result-head__money", card, fmtMoney(val, state.currency));
  } else if (head.kind === "pct") {
    tag("span", "result-head__money", card, fmtPct(Number(head.value) || 0, 2));
  } else if (head.kind === "ratio") {
    tag("span", "result-head__money", card, `1 : ${fmtNum(Number(head.value) || 0, 2)}`);
  } else if (head.kind === "num") {
    tag("span", "result-head__money", card, fmtNum(head.value));
  }
  m.appendChild(card);
}
      ...
      if (getCalc(state.slug).dynamic) {
        ...
        state.defs.forEach((f) => {
          ...
          if (v !== undefined) {
            ...
            state.values[f.key] = f.type === "select" ? v : clamp(Number(v), f.min, f.max);
...
function restoreFromQuery() {
  ...
  state.defs.forEach((f) => {
    ...
    if (f.type === "select") state.values[f.key] = v;
      ...
      const n = Number(v);
      if (Number.isFinite(n)) state.values[f.key] = clamp(n, f.min, f.max);
    }
  });
  return hit;
}
...
function setCurrency(code, skipRender) {
  state.currency = code === "USD" ? "USD" : "INR";
    ...
    if (!restored) {
      ...
      if (last && last.inputs) {
        state.defs.forEach((f) => {
          ...
          else if (v !== undefined) state.values[f.key] = clamp(Number(v), f.min, f.max);
        });
      }
    }
    buildForm();
