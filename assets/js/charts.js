...
/* compact axis labels: 12.3L, 1.05 Cr, 25k, 430 */
...
function renderArea(svg, model, geom) {
  ...
  ticks.forEach((t) => {
    ...
    el("text", { x: padL - 8, y: y + 4, "text-anchor": "end", class: "finchart__tick" }, grid).textContent = fmtAxis(t);
  });
...
function renderDonut(svg, model, geom, curSymbol) {
  ...
  const totalFmt = total >= 1e5 ? fmtAxis(total) : Math.round(total).toLocaleString();
...
/* ============================= BARS ============================= */
function renderBars(svg, model, geom) {
...
export function renderChart(model, container, currencyCode = "INR") {
  ...
  if (model.type === "area") {
    ...
    return chartFrame(model, container, {
      height: 300,
      accessor: (svg, geom) => renderArea(svg, model, geom),
  ...
  if (model.type === "donut") {
    return chartFrame(model, container, {
      height: 300,
      accessor: (svg, geom) => renderDonut(svg, model, geom, symbol),
  ...
  if (model.type === "bars") {
    return chartFrame(model, container, {
      height: 300,
      accessor: (svg, geom) => renderBars(svg, model, geom)
    });
  }
  return null;
}
