...
function sipCompute(o) {
  ...
  const scenario = scenarioSeries(o, "rate", 2, years, (inp, r) => {
    ...
    return { name: `${r}%`, points: labels.map((_, y) => (y === 0 ? 0 : tvm.sipFV(p, Math.min(m, y * 12), r))) };
  });
  const charts = [
...
function realReturnCompute(o) {
  ...
  const real = nominal > 0 ? (1 + nominal / 100) / (1 + inflation / 100) - 1 : -inflation / 100;
  return {
...
function coastFireCompute(o) {
  ...
  let years = 0;
  if (corpus > 0 && target > corpus) years = Math.log(target / corpus) / Math.log(1 + rate / 100);
  const reachable = corpus > 0 && target > corpus;
  ...
  return {
    head: numStat("Coast-FIRE age", reachable ? age + years : null),
    ...
    note: reachable
      ? `Coast FIRE: stop adding money and let compounding do the rest. Your corpus reaches the FI target around age ${fmtNum(age + years, 1)} with no further contributions.`
