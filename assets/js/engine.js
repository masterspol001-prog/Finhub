...
const lineCurve = (id, title, labels, series) => ({
  type: "line", title,
  data: { labels, series: series.map((s) => ({ name: s.name, points: s.points, color: s.color, dashed: !!s.dashed })) }
});

const hbar = (id, title, items) => ({ type: "hbar", title, data: { items } });

const waterfall = (id, title, items) => ({ type: "waterfall", title, data: { items } });

const gauge = (id, title, opts) => ({ type: "gauge", title, data: opts });
...
function gstCompute(o) {
  ...
  const charts = [
    ...
    waterfall("gstBuild", mode === "exclusive" ? "From net price to invoice total" : "From invoice total back to net price", [
      { label: "Net value", value: net, type: "start" },
      { label: "GST", value: tax, type: "delta" },
      { label: "Total", value: total, type: "total", color: "#6366f1" }
    ])
  ];
  return {
...
function investmentReturnCompute(o) {
  ...
  const charts = [
    ...
    waterfall("retBuild", "How your investment became today's value", [
      { label: "Invested", value: totalPut, type: "start" },
      { label: gain >= 0 ? "Gain" : "Loss", value: gain, type: "delta" },
      { label: "Current value", value: final, type: "total", color: "#6366f1" }
    ]),
    hbar("retCompare", "Headline figures", [
      { label: "Amount invested", value: totalPut, color: "#3b82f6", display: fmtMoney(totalPut) },
      { label: "Current value", value: final, color: "#6366f1", display: fmtMoney(final) },
      { label: gain >= 0 ? "Absolute gain" : "Absolute loss", value: Math.abs(gain), color: gain >= 0 ? "#16a34a" : "#ef4444", display: fmtMoney(Math.abs(gain)) }
    ])
  ];
...
function riskRewardCompute(o) {
  ...
  const charts = [
    gauge("rrGauge", undefined, { value: rr, max: Math.max(3, Math.round(rr * 1.5 * 10) / 10), kind: "ratio", label: "Reward : Risk", caption: rr >= 2 ? "Healthy payoff profile" : "Below the 1 : 2 benchmark" }),
    ...
    hbar("rrMoney", qty > 0 ? "Money at risk vs potential profit" : "Distance to stop vs target (per share)", qty > 0 ? [
      { label: "Amount risked", value: riskMoney, color: "#ef4444", display: fmtMoney(riskMoney) },
      { label: "Profit at target", value: winAmt, color: "#16a34a", display: fmtMoney(winAmt) }
    ] : [
      { label: "Risk per share", value: risk, color: "#ef4444", display: fmtMoney(risk) },
      { label: "Reward per share", value: reward, color: "#16a34a", display: fmtMoney(reward) }
    ])
  ];
...
function pnlCompute(o) {
  ...
  const charts = [
    ...
    waterfall("pnlFlow", "From proceeds to net profit", [
      { label: "Proceeds", value: proceeds, type: "start" },
      { label: "Cost", value: -cost, type: "delta" },
      { label: pnl >= 0 ? "Net profit" : "Net loss", value: pnl, type: "total", color: pnl >= 0 ? "#6366f1" : "#ef4444" }
    ])
  ];
  return {
...
function realReturnCompute(o) {
  ...
  return {
    ...
    charts: [
      gauge("rrRealGauge", undefined, { value: real * 100, max: Math.max(nominal, Math.abs(real * 100), 1), kind: "pct", label: "Real return", caption: real >= 0 ? "Ahead of inflation" : "Behind inflation" }),
      hbar("rrRealBars", "Nominal vs inflation vs real", [
        { label: "Nominal return", value: nominal, color: "#3b82f6", display: `${nominal.toFixed(2)}%` },
        { label: "Inflation", value: inflation, color: "#f59e0b", display: `${inflation.toFixed(2)}%` },
        { label: "Real return", value: real * 100, color: real >= 0 ? "#16a34a" : "#ef4444", display: `${(real * 100).toFixed(2)}%` }
      ])
    ],
...
function incomeTaxCompute(o) {
  ...
  return {
    ...
    waterfall("itFlow", "From gross income to take-home", [
      { label: "Gross income", value: income, type: "start" },
      { label: "Tax + cess", value: -total, type: "delta" },
      { label: "Take-home", value: Math.max(0, income - total), type: "total", color: "#16a34a" }
    ]),
    hbar("itParts", "Tax composition", [
      { label: "Taxable income", value: taxable, color: "#3b82f6", display: fmtMoney(taxable) },
      { label: "Base tax", value: base, color: "#ef4444", display: fmtMoney(base) },
      { label: "Cess (4%)", value: cess, color: "#f59e0b", display: fmtMoney(cess) },
      { label: "Total tax", value: total, color: "#ec4899", display: fmtMoney(total) }
    ])],
...
function capitalGainsCompute(o) {
  ...
  return {
    ...
    waterfall("cgFlow", "From gross gain to post-tax gain", [
      { label: "Gross gain", value: gain, type: "start" },
      { label: "Capital gains tax", value: -tax, type: "delta" },
      { label: "Post-tax gain", value: gain - tax, type: "total", color: "#16a34a" }
    ]),
    hbar("cgParts", "Gain breakdown", [
      { label: "Gross gain", value: gain, color: "#3b82f6", display: fmtMoney(gain) },
      { label: type === "stcg" ? "Taxable (full)" : "Taxable (after 1.25L)", value: type === "stcg" ? gain : Math.max(0, gain - 125000), color: "#8b5cf6", display: fmtMoney(type === "stcg" ? gain : Math.max(0, gain - 125000)) },
      { label: "Tax", value: tax, color: "#ef4444", display: fmtMoney(tax) },
      { label: "Post-tax gain", value: gain - tax, color: "#16a34a", display: fmtMoney(gain - tax) }
    ])],
...
/* ============================================================
   Detailed tables
   ------------------------------------------------------------
   Pure derivation of tabular data from a compute() result, so the
   app shell can stay a thin renderer and this logic stays unit-tested.
   Every calculator gains at least one table, plus a totals/final row
   wherever summing or a closing balance is meaningful.
   Cell values are raw; each column carries a `kind` ("money" | "pct" |
   "num" | "years" | "text") that the renderer formats.
   ============================================================ */
const sum = (arr) => {
...
export function deriveTables(result) {
  const out = [];
  if (!result) return out;
  (result.tables || []).forEach((t) => out.push(t));
  (result.charts || []).forEach((ch) => {
    if (!ch || !ch.data) return;
    const d = ch.data;
    if (ch.type === "area" || ch.type === "line") {
      const labels = d.labels || [];
      const series = d.series || [];
      if (labels.length < 2 || !series.length) return;
      out.push({
        title: `${ch.title || "Projection"} - year by year`,
        columns: [{ label: "Period", kind: "text" }].concat(series.map((s) => ({ label: s.name, kind: "money" }))),
        rows: labels.map((lab, i) => [lab].concat(series.map((s) => (s.points && s.points[i] != null ? s.points[i] : null)))),
        note: "Full period-by-period detail behind the chart above."
      });
    } else if (ch.type === "bars") {
      const items = d.items || [];
      if (!items.length) return;
      const split = items.some((it) => it.principal != null || it.interest != null);
      if (split) {
        out.push({
          title: `${ch.title || "Breakdown"} - detailed schedule`,
          columns: [{ label: "Period", kind: "text" }, { label: "Principal", kind: "money" }, { label: "Interest", kind: "money" }, { label: "Total", kind: "money" }, { label: "Balance", kind: "money" }],
          rows: items.map((it) => {
            const p = it.principal || 0, i2 = it.interest || 0;
            return [it.label || it.year, p, i2, p + i2, it.balance != null ? it.balance : p + i2];
          }),
          totals: ["Total", sum(items.map((it) => it.principal || 0)), sum(items.map((it) => it.interest || 0)), sum(items.map((it) => (it.principal || 0) + (it.interest || 0))), items.length ? items[items.length - 1].balance : null]
        });
      } else {
        out.push({
          title: `${ch.title || "Comparison"} - detail`,
          columns: [{ label: "Item", kind: "text" }, { label: "Value", kind: "money" }],
          rows: items.map((it) => [it.label || it.year, it.balance != null ? it.balance : (it.principal || 0) + (it.interest || 0)])
        });
      }
    } else if (ch.type === "hbar") {
      const items = d.items || [];
      if (!items.length) return;
      const allDisplay = items.every((it) => it.display != null);
      out.push({
        title: `${ch.title || "Comparison"} - detail`,
        columns: [{ label: "Item", kind: "text" }, { label: "Value", kind: allDisplay ? "text" : "money" }],
        rows: items.map((it) => [it.label, allDisplay ? it.display : it.value])
      });
    } else if (ch.type === "waterfall") {
      const items = d.items || [];
      if (!items.length) return;
      let run = 0;
      const rows = items.map((it) => {
        if (it.type === "start" || it.type === "total") run = it.value;
        else run += it.value;
        const shown = it.type === "delta" && it.value > 0 ? `+${it.value}` : it.value;
        return [it.label, shown, run];
      });
      out.push({
        title: `${ch.title || "Flow"} - breakdown`,
        columns: [{ label: "Step", kind: "text" }, { label: "Amount", kind: "money" }, { label: "Running", kind: "money" }],
        rows
      });
      ...
      out.push({
        title: `${ch.title || "Breakdown"} - detail`,
        columns: [{ label: "Item", kind: "text" }, { label: "Amount", kind: "money" }, { label: "Share", kind: "pct" }],
        rows: segs.map((s) => [s.name, s.value, total > 0 ? (s.value / total) * 100 : 0]),
...
/* ============================================================
   DECISION ENGINE
   ------------------------------------------------------------
   Everything above answers "what is my number?". This layer answers
   the bigger questions - where am I going, what are my options, what
   should I do next - so the product behaves like an adviser rather
   than a directory of calculators. Pure functions, fully unit-tested.
   ============================================================ */

const n0 = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0);

/* Monthly compounding projection, annuity-due (deposit grows from its own
   month), with optional per-year step-up, a pause window, and extra monthly
   contributions. This is the single workhorse behind every projection here. */
export function projectPlan(input) {
  const o = input || {};
  const monthly = Math.max(0, n0(o.monthly));
  const monthlyExtra = n0(o.monthlyExtra);
  const initial = Math.max(0, n0(o.initial));
  const years = Math.max(0, n0(o.years));
  const returnPct = n0(o.returnPct);
  const stepUpPct = Math.max(-100, n0(o.stepUpPct));
  const inflationPct = n0(o.inflationPct);
  const skipStart = Math.max(0, Math.floor(n0(o.skipStartMonth)));
  const skipMonths = Math.max(0, Math.floor(n0(o.skipMonths)));
  ...
  const corpusSeries = [initial];
  const realSeries = [initial];
  const perYear = [];

  for (let m = 1; m <= months; m++) {
    const yrIdx = Math.floor((m - 1) / 12);
    const pmt = Math.max(0, (monthly + monthlyExtra) * Math.pow(1 + stepUpPct / 100, yrIdx));
    const paused = skipMonths > 0 && m > skipStart && m <= skipStart + skipMonths;
    if (!paused) { bal += pmt; invested += pmt; yearPmt += pmt; }
    bal *= 1 + i;
    if (m % 12 === 0) {
      const y = m / 12;
      const interest = bal - yearStart - yearPmt;
      perYear.push({ year: y, principal: yearPmt, interest, balance: bal });
      labels.push(`Yr ${y}`);
      investedSeries.push(invested);
      corpusSeries.push(bal);
      realSeries.push(bal / Math.pow(1 + inflationPct / 100, y));
      yearStart = bal;
      yearPmt = 0;
    }
  }
  const yearsActual = months / 12;
  const real = bal / Math.pow(1 + inflationPct / 100, yearsActual);
  return {
    months, years: yearsActual, final: bal, real, invested,
    gains: bal - invested, labels, investedSeries, corpusSeries, realSeries, perYear
  };
}

/* Inflation-adjusted equivalent of a nominal amount. */
export function inflate(amount, inflationPct, years) {
  return n0(amount) * Math.pow(1 + n0(inflationPct) / 100, Math.max(0, n0(years)));
}

/* Total own-money contributed by a step-up SIP over its life. */
export function sipInvested(monthly, nMonths, stepUpPct) {
  const g = Math.max(0, n0(stepUpPct)) / 100;
  let total = 0;
  for (let m = 1; m <= nMonths; m++) total += n0(monthly) * Math.pow(1 + g, Math.floor((m - 1) / 12));
  return total;
}

/* Required monthly (annuity-due, no step-up) to close a future-value gap. */
export function requiredMonthly(gap, nMonths, returnPct) {
  const f = n0(tvm.sipFactor(nMonths, n0(returnPct)));
  if (f <= 0) return 0;
  return Math.max(0, n0(gap)) / f;
}

/* Binary-search the step-up % (0..cap) that makes a deposit stream hit a
   target future value. Returns 0 when the target is already met. */
export function solveStepUp(monthly, years, returnPct, target) {
  if (n0(monthly) <= 0 || n0(years) <= 0) return 0;
  const at = (step) => tvm.sipStepUpFV(n0(monthly), Math.round(n0(years) * 12), n0(returnPct), step);
  if (at(0) >= n0(target)) return 0;
  let lo = 0, hi = 40;
  if (at(hi) < n0(target)) return null; // even 40% is not enough
  for (let k = 0; k < 40; k++) {
    const mid = (lo + hi) / 2;
    if (at(mid) >= n0(target)) hi = mid; else lo = mid;
  }
  return Math.ceil(hi * 10) / 10;
}

/* Binary-search the years needed for a plan to hit an inflating goal. */
export function solveYears(input, maxYears = 80) {
  for (let y = 0; y <= maxYears; y += 0.5) {
    const target = inflate(n0(input.goalAmount), input.inflationPct, y);
    const proj = projectPlan({ ...input, years: y });
    if (proj.final >= target && proj.final > 0) return y;
  }
  return null;
}

/* ---- Goal planner -------------------------------------------------------
   Answers: how big must the goal be in future rupees, am I on track, how
   much must I invest, and how long will it take at my current plan. */
export function goalPlan(input) {
  const o = input || {};
  const goalAmount = Math.max(0, n0(o.goalAmount));
  const years = Math.max(0, n0(o.years));
  const inflationPct = n0(o.inflationPct);
  const returnPct = n0(o.returnPct);
  const currentSavings = Math.max(0, n0(o.currentSavings));
  const monthly = Math.max(0, n0(o.monthly));
  const stepUpPct = Math.max(0, n0(o.stepUpPct));

  const futureGoal = inflate(goalAmount, inflationPct, years);
  const proj = projectPlan({ monthly, monthlyExtra: 0, initial: currentSavings, years, returnPct, stepUpPct, inflationPct });
  const projected = proj.final;
  const shortfall = Math.max(0, futureGoal - projected);
  const surplus = Math.max(0, projected - futureGoal);
  const goalAchieved = futureGoal <= 0 || projected >= futureGoal;
  const progress = futureGoal > 0 ? Math.min(1, projected / futureGoal) : 1;

  /* required monthly, counting growth on the money you already have */
  const grownSavings = currentSavings * Math.pow(1 + returnPct / 100, years);
  const gap = Math.max(0, futureGoal - grownSavings);
  const reqMonthly = requiredMonthly(gap, Math.round(years * 12), returnPct);

  /* required step-up at the current monthly contribution */
  const stepUpNeeded = goalAchieved ? 0 : solveStepUp(monthly, years, returnPct, demandFuture(goalAmount, currentSavings, returnPct, inflationPct, years));
  const yearsToGoal = solveYears({ monthly, initial: currentSavings, returnPct, stepUpPct, inflationPct, goalAmount }, 80);

  return {
    goalAmount, futureGoal, projected, realProjected: proj.real, invested: proj.invested,
    gains: proj.gains, shortfall, surplus, goalAchieved, progress,
    currentProgress: goalAmount > 0 ? Math.min(1, currentSavings / goalAmount) : 1,
    requiredMonthly: reqMonthly, requiredStepUp: stepUpNeeded, yearsToGoal,
    inputs: { goalAmount, years, inflationPct, returnPct, currentSavings, monthly, stepUpPct },
    project: proj
  };
}

/* Future value still needed after the current savings grow at the expected
   return, used when solving for step-up / monthly contribution. */
function demandFuture(goalAmount, currentSavings, returnPct, inflationPct, years) {
  const futureGoal = inflate(goalAmount, inflationPct, years);
  const grown = n0(currentSavings) * Math.pow(1 + n0(returnPct) / 100, Math.max(0, n0(years)));
  return Math.max(0, futureGoal - grown);
}

/* ---- Strategy generator -------------------------------------------------
   Given a plan and a goal, produce concrete, comparable ways to hit it:
   pay more, step up, invest longer, or start with more. Each strategy is
   costed so the recommendation is honest about the trade-off. */
export function buildStrategies(input) {
  const o = input || {};
  const base = {
    goalAmount: Math.max(0, n0(o.goalAmount)),
    years: Math.max(0, n0(o.years)),
    inflationPct: n0(o.inflationPct),
    returnPct: n0(o.returnPct),
    currentSavings: Math.max(0, n0(o.currentSavings)),
    monthly: Math.max(0, n0(o.monthly)),
    stepUpPct: Math.max(0, n0(o.stepUpPct))
  };
  const futureGoal = inflate(base.goalAmount, base.inflationPct, base.years);
  const grownSavings = base.currentSavings * Math.pow(1 + base.returnPct / 100, base.years);
  const gap = Math.max(0, futureGoal - grownSavings);

  const mk = (id, label, why, patch, extra = {}) => {
    const plan = { ...base, ...patch };
    const proj = projectPlan({ monthly: plan.monthly, monthlyExtra: 0, initial: plan.currentSavings, years: plan.years, returnPct: plan.returnPct, stepUpPct: plan.stepUpPct, inflationPct: base.inflationPct });
    const totalInvested = proj.invested;
    return {
      id, label, why,
      monthly: plan.monthly, currentSavings: plan.currentSavings, years: plan.years, stepUpPct: plan.stepUpPct, returnPct: plan.returnPct,
      projected: proj.final, realProjected: proj.real, invested: totalInvested, gains: proj.gains,
      goalAchieved: futureGoal <= 0 || proj.final >= futureGoal,
      shortfall: Math.max(0, futureGoal - proj.final),
      extraMonthly: Math.max(0, plan.monthly - base.monthly),
      extraInitial: Math.max(0, plan.currentSavings - base.currentSavings),
      extraYears: Math.max(0, plan.years - base.years),
      stepUpDelta: Math.max(0, plan.stepUpPct - base.stepUpPct),
      extraOutlay: totalInvested - projectPlan({ monthly: base.monthly, initial: base.currentSavings, years: base.years, returnPct: base.returnPct, stepUpPct: base.stepUpPct, inflationPct: base.inflationPct }).invested,
      ...extra
    };
  };

  const strategies = [];
  strategies.push(mk("current", "Current plan", "Where you are today - the honest baseline.", {}));

  // 1. Increase the monthly SIP (kill any step-up for a clean comparison)
  const reqMonthly = requiredMonthly(gap, Math.round(base.years * 12), base.returnPct);
  if (reqMonthly > base.monthly) {
    strategies.push(mk("increase-sip", "Increase monthly SIP", `Raise the instalment to close the gap without extending the timeline.`, { monthly: Math.ceil(reqMonthly / 100) * 100, stepUpPct: 0 }));
  }

  // 2. Step up the SIP every year (same starting instalment)
  const stepNeeded = solveStepUp(base.monthly, base.years, base.returnPct, gap);
  const stepTry = stepNeeded != null ? stepNeeded : 15;
  if (stepTry > 0) {
    strategies.push(mk("step-up", "Step up every year", `Keep the starting instalment modest and let it grow with your income.`, { stepUpPct: Math.max(base.stepUpPct, Math.ceil(stepTry)) }));
  }

  // 3. Invest for longer
  const yNeeded = solveYears({ monthly: base.monthly, initial: base.currentSavings, returnPct: base.returnPct, stepUpPct: base.stepUpPct, inflationPct: base.inflationPct, goalAmount: base.goalAmount }, 80);
  if (yNeeded != null && yNeeded > base.years) {
    strategies.push(mk("longer", "Invest for longer", `Give compounding more time; the monthly commitment stays the same.`, { years: Math.ceil(yNeeded * 2) / 2 }));
  }

  // 4. Start with a bigger lump sum
  const lumpNeeded = base.returnPct === 0 ? gap : gap / Math.pow(1 + base.returnPct / 100, base.years);
  if (lumpNeeded > base.currentSavings) {
    strategies.push(mk("lump-sum", "Invest a larger amount up front", `A bigger starting balance earns from day one, reducing the monthly burden.`, { currentSavings: Math.ceil(lumpNeeded / 1000) * 1000 }));
  }

  // Recommendation: the cheapest strategy (least extra own money) that reaches
  // the goal. If you are already on track, recommend a modest step-up buffer.
  const achievers = strategies.filter((s) => s.id !== "current" && s.goalAchieved);
  let recommendedId = "current";
  if (!strategies[0].goalAchieved && achievers.length) {
    recommendedId = achievers.reduce((best, s) => (s.extraOutlay < best.extraOutlay ? s : best), achievers[0]).id;
  } else if (strategies[0].goalAchieved) {
    recommendedId = strategies.find((s) => s.id === "step-up") ? "step-up" : "current";
  }
  strategies.forEach((s) => { s.recommended = s.id === recommendedId; });

  return { base, futureGoal, gap, strategies, recommendedId };
}

/* ---- Scenario comparison (2-5 SIP / lump-sum plans side by side) -------- */
export function compareScenarios(scenarios, opts) {
  const o = opts || {};
  const inflationPct = n0(o.inflationPct);
  const goalAmount = Math.max(0, n0(o.goalAmount));
  const rows = (scenarios || []).map((s, idx) => {
    const years = Math.max(0, n0(s.years));
    const proj = projectPlan({
      monthly: Math.max(0, n0(s.monthly)), initial: Math.max(0, n0(s.initial)),
      years, returnPct: n0(s.returnPct), stepUpPct: Math.max(0, n0(s.stepUpPct)), inflationPct
    });
    const totalInvested = proj.invested;
    const gains = proj.final - totalInvested;
    const cagr = totalInvested > 0 && years > 0 ? Math.pow(proj.final / totalInvested, 1 / years) - 1 : 0;
    const futureGoal = goalAmount > 0 ? inflate(goalAmount, inflationPct, years) : 0;
    return {
      id: s.id || `s${idx + 1}`,
      name: s.name || `Scenario ${String.fromCharCode(65 + idx)}`,
      monthly: Math.max(0, n0(s.monthly)), initial: Math.max(0, n0(s.initial)),
      stepUpPct: Math.max(0, n0(s.stepUpPct)), years, returnPct: n0(s.returnPct),
      projected: proj.final, realProjected: proj.real, invested: totalInvested, gains,
      cagrPct: cagr * 100,
      goalAmount, futureGoal,
      goalAchieved: futureGoal <= 0 ? null : proj.final >= futureGoal,
      shortfall: futureGoal > 0 ? Math.max(0, futureGoal - proj.final) : 0,
      surplus: futureGoal > 0 ? Math.max(0, proj.final - futureGoal) : 0,
      series: proj.corpusSeries, investedSeries: proj.investedSeries,
      perYear: proj.perYear
    };
  });
  const bestCorpus = rows.length ? Math.max(...rows.map((r) => r.projected)) : 0;
  const bestGains = rows.length ? Math.max(...rows.map((r) => r.gains)) : 0;
  rows.forEach((r) => {
    r.highestCorpus = r.projected === bestCorpus && bestCorpus > 0;
    r.highestGains = r.gains === bestGains && bestGains > 0;
  });
  return { rows, inflationPct, goalAmount };
}

/* ---- Savings planner ---------------------------------------------------- */
export function savingsPlan(input) {
  const o = input || {};
  const income = Math.max(0, n0(o.income));
  const fixed = Math.max(0, n0(o.fixed));
  const variable = Math.max(0, n0(o.variable));
  const debt = Math.max(0, n0(o.debt));
  const monthlyInvestment = Math.max(0, n0(o.monthlyInvestment));
  const savings = Math.max(0, n0(o.savings));
  const years = Math.max(0, n0(o.years));
  const returnPct = n0(o.returnPct);
  const inflationPct = n0(o.inflationPct);

  const expenses = fixed + variable;
  const outflow = expenses + debt;
  const cashFlow = income - outflow;
  const surplus = cashFlow - monthlyInvestment;
  ...
  const emergencyTarget = 6 * outflow;
  const emergencyMonths = outflow > 0 ? savings / outflow : 0;
  const emergencyGap = Math.max(0, emergencyTarget - savings);
  const extraInvestable = Math.max(0, surplus);

  const bump = [0, 1000, 3000, 5000].map((extra) => {
    const proj = projectPlan({ monthly: monthlyInvestment + extra, monthlyExtra: 0, initial: 0, years, returnPct, inflationPct });
    return { extra, monthly: monthlyInvestment + extra, projected: proj.final, real: proj.real, invested: proj.invested, gains: proj.gains };
  });

  return {
    income, expenses, debt, outflow, cashFlow, surplus, savingsRate, investmentRate,
    expenseRatio, debtRatio, emergencyTarget, emergencyMonths, emergencyGap,
    extraInvestable, monthlyInvestment, scenarios: bump, years, returnPct, inflationPct
  };
}

/* ---- Financial health score --------------------------------------------
   Transparent, capped, weighted components. This is a coaching gauge, not a
   credit score: each component shows exactly how it was earned. */
export function healthScore(input) {
  const o = input || {};
  const income = Math.max(0, n0(o.income));
  const expenses = Math.max(0, n0(o.fixed)) + Math.max(0, n0(o.variable));
  const debt = Math.max(0, n0(o.debt));
  const monthlyInvestment = Math.max(0, n0(o.monthlyInvestment));
  const savings = Math.max(0, n0(o.savings));
  const progress = Math.max(0, Math.min(1, n0(o.goalProgress)));

  const outflow = expenses + debt;
  const cashFlow = income - outflow;
  const savingsRate = income > 0 ? Math.max(0, cashFlow) / income * 100 : 0;
  const investRate = income > 0 ? monthlyInvestment / income * 100 : 0;
  const debtRatio = income > 0 ? debt / income * 100 : 0;
  const emergencyMonths = outflow > 0 ? savings / outflow : (savings > 0 ? 12 : 0);

  const cap = (v) => Math.max(0, Math.min(100, v));
  const components = [
    { key: "savings", label: "Savings rate", weight: 25, score: cap((savingsRate / 30) * 100), value: savingsRate, kind: "pct", detail: "Target: 30% of income left after spending and debt." },
    { key: "emergency", label: "Emergency fund", weight: 20, score: cap((emergencyMonths / 6) * 100), value: emergencyMonths, kind: "months", detail: "Target: 6 months of expenses set aside in cash." },
    { key: "investing", label: "Investment rate", weight: 20, score: cap((investRate / 20) * 100), value: investRate, kind: "pct", detail: "Target: 20% of income invested every month." },
    { key: "debt", label: "Debt burden", weight: 20, score: cap(((40 - debtRatio) / 40) * 100), value: debtRatio, kind: "pct", detail: "EMIs below 40% of income; lower is better." },
    { key: "goal", label: "Goal progress", weight: 15, score: cap(progress * 100), value: progress * 100, kind: "pct", detail: "How far your projected corpus is toward your goal." }
  ];
  const score = Math.round(components.reduce((a, c) => a + c.score * c.weight, 0) / 100);
  const grade = score >= 85 ? "Excellent" : score >= 70 ? "Strong" : score >= 55 ? "Fair" : score >= 40 ? "Needs work" : "At risk";

  // weakest components first => what to improve next
  const improvements = components
    .filter((c) => c.score < 85)
    .sort((a, b) => a.score - b.score)
    .map((c) => ({ key: c.key, label: c.label, score: Math.round(c.score), detail: c.detail }));

  return { score, grade, components: components.map((c) => ({ ...c, score: Math.round(c.score) })), improvements, savingsRate, investRate, debtRatio, emergencyMonths, cashFlow };
}

/* ---- What-if lab --------------------------------------------------------
   Each lever perturbs one assumption at a time so the user can feel the
   sensitivity of the plan. */
export const WHAT_IF_LEVERS = [
  { id: "raise_sip", label: "Increase monthly SIP", kind: "amount", def: 2000, detail: "Add to every monthly instalment." },
  { id: "step_up", label: "Step up every year", kind: "pct", def: 10, detail: "Raise the instalment annually with your income." },
  { id: "extend", label: "Invest longer", kind: "years", def: 2, detail: "Extend the horizon by a couple of years." },
  { id: "lump_sum", label: "Add a lump sum today", kind: "amount", def: 100000, detail: "Bring forward a one-off investment." },
  { id: "lower_return", label: "Assume a lower return", kind: "deltaPct", def: -2, detail: "Stress-test a weaker market." },
  { id: "inflation", label: "Inflation runs higher", kind: "deltaPct", def: 2, realDelta: true, detail: "See how higher inflation shrinks the real value of the same corpus." },
  { id: "pause", label: "Pause investing for 1 year", kind: "none", def: 0, detail: "Skip 12 monthly deposits, e.g. after a job change." },
  { id: "retire_early", label: "Finish 2 years earlier", kind: "years", def: -2, detail: "Shorten the horizon to reach the goal sooner." }
];

export function whatIf(base, leverId, amount) {
  const b = {
    monthly: Math.max(0, n0(base.monthly)), monthlyExtra: 0, initial: Math.max(0, n0(base.initial)),
    years: Math.max(0, n0(base.years)), returnPct: n0(base.returnPct), stepUpPct: Math.max(0, n0(base.stepUpPct)),
    inflationPct: n0(base.inflationPct)
  };
  const lever = WHAT_IF_LEVERS.find((l) => l.id === leverId) || WHAT_IF_LEVERS[0];
  const v = amount == null ? lever.def : n0(amount);
  const modified = { ...b };
  switch (leverId) {
    case "raise_sip": modified.monthlyExtra = v; break;
    case "step_up": modified.stepUpPct = b.stepUpPct + v; break;
    case "extend": modified.years = Math.max(0, b.years + v); break;
    case "lump_sum": modified.initial = b.initial + v; break;
    case "lower_return": modified.returnPct = b.returnPct + v; break;
    case "inflation": modified.inflationPct = Math.max(0, b.inflationPct + v); break;
    case "pause": modified.skipStartMonth = Math.max(0, Math.min(24, Math.round(b.years * 12 / 2))); modified.skipMonths = 12; break;
    case "retire_early": modified.years = Math.max(0, b.years + v); break;
    default: break;
  }
  const before = projectPlan({ ...b });
  const after = projectPlan({ ...modified });
  const goalAmount = Math.max(0, n0(base.goalAmount));
  const goalFuture = goalAmount > 0 ? inflate(goalAmount, b.inflationPct, b.years) : 0;
  return {
    lever, amount: v,
    base: { projected: before.final, invested: before.invested, real: before.real },
    modified: { projected: after.final, invested: after.invested, real: after.real },
    deltaCorpus: after.final - before.final,
    deltaReal: after.real - before.real,
    deltaInvested: after.invested - before.invested,
    achievedBefore: goalFuture > 0 ? before.final >= goalFuture : null,
    achievedAfter: goalFuture > 0 ? after.final >= goalFuture : null,
    goalFuture,
    before, after
  };
}
...
/* =========================================================================
   DECISION PLATFORM PLANNERS
   -------------------------------------------------------------------------
   Pure, side-effect-free functions behind the dashboards and planners. Each
   one documents its assumptions so the UI can show them honestly and the test
   suite can pin the mathematics. No AI, no advice - just arithmetic.
   ========================================================================= */

/* ---- Retirement planner ------------------------------------------------
   Corpus needed = present value of an inflation-growing annual withdrawal
   stream over the retirement years. Saving side uses an annuity-due SIP and
   lumps the current corpus in at the start. */
export function retirementPlan(input) {
  const o = input || {};
  const ageNow = Math.max(15, Math.round(n0(o.ageNow) || 30));
  const ageRet = Math.max(ageNow + 1, Math.round(n0(o.ageRet) || 60));
  const lifeExp = Math.max(ageRet + 1, Math.round(n0(o.lifeExp) || 85));
  const monthlyExpense = Math.max(0, n0(o.monthlyExpense) || 60000);
  const inflationPct = n0(o.inflationPct);
  const returnPct = n0(o.returnPct);
  const wdReturnPct = n0(o.wdReturnPct);
  const corpusNow = Math.max(0, n0(o.corpusNow));
  const monthlySave = Math.max(0, n0(o.monthlySave));

  const build = (aRet, mSave, ret) => {
    const yearsToRetire = Math.max(0, aRet - ageNow);
    const yearsInRetirement = Math.max(1, lifeExp - aRet);
    const futureMonthlyExpense = monthlyExpense * Math.pow(1 + inflationPct / 100, yearsToRetire);
    const corpusNeeded = growingAnnuityDuePV(futureMonthlyExpense * 12, inflationPct, wdReturnPct, yearsInRetirement);
    const projected = tvm.fv(corpusNow, ret, yearsToRetire) + tvm.sipFV(mSave, Math.round(yearsToRetire * 12), ret);
    const gap = Math.max(0, corpusNeeded - projected);
    const factor = tvm.sipFactor(Math.round(yearsToRetire * 12), ret);
    const requiredMonthly = factor > 0 ? gap / factor : 0;
    return {
      ageRet: aRet, yearsToRetire, yearsInRetirement, futureMonthlyExpense, corpusNeeded,
      projected, gap, requiredMonthly, onTrack: projected >= corpusNeeded,
      progress: corpusNeeded > 0 ? Math.min(1, projected / corpusNeeded) : 1,
      monthlySave: mSave, returnPct: ret
    };
  };

  const current = build(ageRet, monthlySave, returnPct);
  const scenarios = [
    { id: "current", label: "Current plan", detail: `Retire at ${ageRet}, saving ${fmtMoney(monthlySave)} a month.`, ...current },
    { id: "later", label: `Retire ${5} years later`, detail: `More saving years and fewer retirement years.`, ...build(ageRet + 5, monthlySave, returnPct) },
    { id: "earlier", label: "Retire 5 years earlier", detail: "Fewer saving years and a longer retirement.", ...build(Math.max(ageNow + 1, ageRet - 5), monthlySave, returnPct) },
    { id: "save-more", label: "Save 25% more each month", detail: `Raise the monthly saving from ${fmtMoney(monthlySave)} to ${fmtMoney(monthlySave * 1.25)}.`, ...build(ageRet, monthlySave * 1.25, returnPct) },
    { id: "lower-return", label: "Assume a 2% lower return", detail: "Stress-test a weaker market before you commit.", ...build(ageRet, monthlySave, Math.max(0, returnPct - 2)) }
  ];

  const yearsToPlot = Math.max(1, Math.round(lifeExp - ageNow));
  const labels = [];
  const corpus = [];
  for (let y = 0; y <= yearsToPlot; y++) {
    labels.push(`Age ${ageNow + y}`);
    if (y <= current.yearsToRetire) {
      corpus.push(tvm.fv(corpusNow, returnPct, y) + tvm.sipFV(monthlySave, y * 12, returnPct));
    } else {
      corpus.push(0);
    }
  }
  let pot = corpus[Math.round(current.yearsToRetire)] || 0;
  const retireIdx = Math.round(current.yearsToRetire);
  for (let y = retireIdx + 1; y <= yearsToPlot; y++) {
    const s = y - retireIdx - 1;
    const wd = current.futureMonthlyExpense * 12 * Math.pow(1 + inflationPct / 100, s);
    pot = Math.max(0, (pot - wd) * (1 + wdReturnPct / 100));
    corpus[y] = pot;
  }

  return {
    ageNow, ageRet, lifeExp, monthlyExpense, inflationPct, returnPct, wdReturnPct, corpusNow, monthlySave,
    ...current,
    curve: { labels, corpus, target: current.corpusNeeded, retireAge: ageRet },
    scenarios
  };
}

/* ---- Debt planner -------------------------------------------------------
   Monthly simulation. Every month: interest accrues, minimum payments are
   made, then any extra is thrown at one target debt - highest APR first
   (avalanche) or smallest balance first (snowball). Cleared minimums roll
   into the next target, the classic snowball effect. */
export function simulateDebts(debts, extra, strategy) {
  const list = (debts || [])
    .map((d, i) => ({
      id: d.id || `d${i + 1}`,
      name: d.name || `Debt ${i + 1}`,
      balance: Math.max(0, n0(d.balance)),
      apr: Math.max(0, n0(d.apr)),
      payment: Math.max(0, n0(d.payment))
    }))
    .filter((d) => d.balance > 0);

  const order = [...list].sort((a, b) => (strategy === "snowball" ? a.balance - b.balance : b.apr - a.apr));
  const bal = list.map((d) => ({ ...d, bal: d.balance }));
  const minTotal = list.reduce((s, d) => s + d.payment, 0);
  const pmtTotal = minTotal + Math.max(0, n0(extra));
  const schedule = [];
  const clearedAt = {};
  let months = 0;
  let totalInterest = 0;
  ...
  const outstanding = () => bal.reduce((s, b) => s + Math.max(0, b.bal), 0);
  while (outstanding() > 0.5 && months < MAX) {
    months++;
    let available = pmtTotal;
    bal.forEach((b) => {
      if (b.bal <= 0) return;
      const int = b.bal * (b.apr / 100 / 12);
      b.bal += int;
      totalInterest += int;
    });
    bal.forEach((b) => {
      if (b.bal <= 0) return;
      const pay = Math.min(b.bal, b.payment, available);
      b.bal -= pay; available -= pay; totalPaid += pay;
      if (b.bal <= 0.5) { b.bal = 0; if (clearedAt[b.id] == null) clearedAt[b.id] = months; }
    });
    for (const target of order) {
      if (available <= 0.5) break;
      const b = bal.find((x) => x.id === target.id);
      if (!b || b.bal <= 0) continue;
      const pay = Math.min(b.bal, available);
      b.bal -= pay; available -= pay; totalPaid += pay;
      if (b.bal <= 0.5) { b.bal = 0; if (clearedAt[b.id] == null) clearedAt[b.id] = months; }
    }
    if (months % 12 === 0 || outstanding() <= 0.5) {
      schedule.push({ month: months, balance: Math.round(outstanding()) });
    }
  }
  const cleared = outstanding() <= 0.5;
  return {
    months: cleared ? months : null,
    totalInterest, totalPaid, minTotal, pmtTotal, schedule,
    order: order.map((d) => ({ id: d.id, name: d.name })),
    clearedAt,
    balances: bal.map((b) => ({ id: b.id, name: b.name, balance: b.balance, apr: b.apr, payment: b.payment }))
  };
}

export function debtPlan(input) {
  const o = input || {};
  const strategy = o.strategy === "snowball" ? "snowball" : "avalanche";
  const debts = (o.debts || []).map((d, i) => ({
    id: d.id || `d${i + 1}`,
    name: d.name || `Debt ${i + 1}`,
    balance: Math.max(0, n0(d.balance)),
    apr: Math.max(0, n0(d.apr)),
  ...
  const extra = Math.max(0, n0(o.extra));

  const totalBalance = debts.reduce((s, d) => s + d.balance, 0);
  const minTotal = debts.reduce((s, d) => s + d.payment, 0);
  const base = simulateDebts(debts, 0, strategy);
  const current = simulateDebts(debts, extra, strategy);
  const avalanche = simulateDebts(debts, extra, "avalanche");
  const snowball = simulateDebts(debts, extra, "snowball");

  const scenarios = [0, 2000, 5000, 10000].map((ex) => {
    const s = simulateDebts(debts, ex, strategy);
    return {
      extra: ex, months: s.months, totalInterest: s.totalInterest, totalPaid: s.totalPaid,
      interestSaved: Math.max(0, base.totalInterest - s.totalInterest),
      monthsSaved: s.months != null && base.months != null ? Math.max(0, base.months - s.months) : 0,
      onTrack: s.months != null
    };
  });

  return {
    debts, strategy, extra, totalBalance, minTotal, monthlyBudget: minTotal + extra,
    months: current.months, totalInterest: current.totalInterest, totalPaid: current.totalPaid,
    schedule: current.schedule, order: current.order, clearedAt: current.clearedAt,
    ...
    canClear: current.months != null
  };
}

/* ---- Emergency fund planner -------------------------------------------- */
export function emergencyPlan(input) {
  const o = input || {};
  const essentialMonthly = Math.max(0, n0(o.essentialMonthly));
  const coverMonths = clamp(Math.round(n0(o.coverMonths) || 6), 1, 24);
  const current = Math.max(0, n0(o.current));
  const monthly = Math.max(0, n0(o.monthly));
  const ratePct = n0(o.ratePct);

  const target = essentialMonthly * coverMonths;
  const coverageMonths = essentialMonthly > 0 ? current / essentialMonthly : 0;
  const gap = Math.max(0, target - current);
  const monthsToFill = gap <= 0 ? 0 : solveMonthsTo(monthly, ratePct, current, target);
  const reachable = isFinite(monthsToFill);

  const horizons = [3, 6, 9, 12].map((m) => {
    const t = essentialMonthly * m;
    const g = Math.max(0, t - current);
    const mo = g <= 0 ? 0 : solveMonthsTo(monthly, ratePct, current, t);
    return { months: m, target: t, gap: g, monthsToFill: isFinite(mo) ? mo : null };
  });

  const years = reachable ? Math.max(1, Math.ceil(monthsToFill / 12) + 1) : 6;
  const labels = yearLabels(years);
  const values = labels.map((_, y) => current * Math.pow(1 + ratePct / 100, y) + tvm.sipFV(monthly, y * 12, ratePct));

  return {
    essentialMonthly, coverMonths, current, monthly, ratePct,
    target, coverageMonths, gap, monthsToFill: reachable ? monthsToFill : null,
    reachable, horizons,
    projection: { labels, values, target }
  };
}

/* ---- Net worth ---------------------------------------------------------- */
export function netWorthTotals(input) {
  const o = input || {};
  const clean = (arr) => (arr || []).map((a, i) => ({ id: a.id || `x${i + 1}`, name: a.name || "Item", value: Math.max(0, n0(a.value)) })).filter((a) => a.value > 0);
  const assets = clean(o.assets);
  const liabilities = clean(o.liabilities);
  const totalAssets = assets.reduce((s, a) => s + a.value, 0);
  const totalLiabilities = liabilities.reduce((s, a) => s + a.value, 0);
  const netWorth = totalAssets - totalLiabilities;
  const palette = ["#3b82f6", "#16a34a", "#8b5cf6", "#f59e0b", "#06b6d4", "#ec4899", "#14b8a6", "#f43f5e"];
  const allocation = assets.map((a, i) => ({ name: a.name, value: a.value, color: palette[i % palette.length] }));
  return {
    assets, liabilities, totalAssets, totalLiabilities, netWorth,
    debtToAsset: totalAssets > 0 ? totalLiabilities / totalAssets * 100 : (totalLiabilities > 0 ? 100 : 0),
    liquidsShare: totalAssets > 0 ? (assets.filter((a) => /cash|bank|fd|deposit|savings|liquid/i.test(a.name)).reduce((s, a) => s + a.value, 0) / totalAssets) * 100 : 0,
    allocation
  };
}

export function netWorthTrend(snapshots) {
  const snaps = (snapshots || [])
    .filter((s) => s && Number.isFinite(Number(s.netWorth)) && s.date)
    .map((s) => ({ date: String(s.date), netWorth: Number(s.netWorth), totalAssets: n0(s.totalAssets), totalLiabilities: n0(s.totalLiabilities) }))
    .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
  const labels = snaps.map((s) => s.date);
  const values = snaps.map((s) => s.netWorth);
  const first = values.length ? values[0] : 0;
  const latest = values.length ? values[values.length - 1] : 0;
  const change = latest - first;
  return {
    snapshots: snaps, labels, values,
    first, latest, change,
    changePct: first !== 0 ? (change / Math.abs(first)) * 100 : 0,
    high: values.length ? Math.max(...values) : 0,
    low: values.length ? Math.min(...values) : 0
  };
}

/* ---- SIP strategy presets ---------------------------------------------- */
export function sipStrategies(input) {
  const o = input || {};
  const monthly = Math.max(0, n0(o.monthly));
  const initial = Math.max(0, n0(o.initial));
  const years = Math.max(0, n0(o.years));
  ...
  const defs = [
    { id: "flat", label: "Flat SIP", why: "The same amount every month.", monthly, initial, stepUpPct: 0 },
    { id: "step5", label: "Step up 5% a year", why: "A gentle raise that tracks income growth.", monthly, initial, stepUpPct: 5 },
    { id: "step10", label: "Step up 10% a year", why: "Starts low, grows fast - powerful over long horizons.", monthly, initial, stepUpPct: 10 },
    { id: "higher", label: "25% higher SIP", why: "More every month with no step-up to manage.", monthly: monthly * 1.25, initial, stepUpPct: 0 },
    { id: "lump", label: "SIP + one year up front", why: "Add a lump sum equal to 12 instalments today.", monthly, initial: initial + monthly * 12, stepUpPct: 0 }
  ];
  const cmp = compareScenarios(defs.map((d) => ({ ...d, years, returnPct })), { inflationPct, goalAmount });
  cmp.rows.forEach((r, i) => { r.meta = defs[i]; r.extraOutlay = r.invested - (cmp.rows[0] ? cmp.rows[0].invested : 0); });
  const best = cmp.rows.reduce((a, b) => (b.projected > a.projected ? b : a), cmp.rows[0] || {});
  const reached = goalAmount > 0 ? cmp.rows.filter((r) => r.goalAchieved === true) : [];
  const byCheapest = (a, b) => (a.extraOutlay - b.extraOutlay) || (b.projected - a.projected);
  const recommended = reached.length
    ? [...reached].sort(byCheapest)[0]
    : goalAmount > 0
      ? best
      : [...cmp.rows].sort(byCheapest)[0];
  return {
    rows: cmp.rows, bestId: best ? best.id : null, recommendedId: recommended ? recommended.id : null,
    reachedGoal: reached.length > 0, inflationPct, goalAmount
  };
}

/* ---- Tools registry + journeys ----------------------------------------- */
/* ---- One-shot financial snapshot ----------------------------------------
   The "start with your numbers" layer. Takes a handful of plain numbers and
   turns them into the few things a person actually wants to know: what is
   left each month, how safe they are, whether retirement is on track, what
   they own net of debts, and how much debt pressure they carry. Pure math -
   no advice - and every figure is traceable to the inputs above it. */
export function financialSnapshot(input) {
  const o = input || {};
  const age = Math.max(15, Math.round(n0(o.age) || 30));
  const income = Math.max(0, n0(o.income));
  const expenses = Math.max(0, n0(o.expenses));
  const cash = Math.max(0, n0(o.cash));
  const investments = Math.max(0, n0(o.investments));
  const debt = Math.max(0, n0(o.debt));
  const debtPayment = Math.max(0, n0(o.debtPayment));
  const returnPct = o.returnPct == null ? 12 : n0(o.returnPct);
  const inflationPct = o.inflationPct == null ? 6 : n0(o.inflationPct);
  const wdReturnPct = o.wdReturnPct == null ? 7 : n0(o.wdReturnPct);
  const ageRet = Math.max(age + 1, Math.round(n0(o.ageRet) || 60));
  ...
  const emergencyMonths = expenses > 0 ? cash / expenses : (cash > 0 ? 12 : 0);
  const emergencyGap = Math.max(0, emergencyTarget - cash);

  const retirement = retirementPlan({
    ageNow: age, ageRet, monthlyExpense: expenses, corpusNow: investments,
    monthlySave: Math.max(0, surplus), returnPct, inflationPct, wdReturnPct
  });
  const health = healthScore({
    income, fixed: expenses, variable: 0, debt: debtPayment,
    monthlyInvestment: Math.max(0, surplus), savings: cash, goalProgress: retirement.progress
  });
  const surplus10y = projectPlan({ monthly: Math.max(0, surplus), initial: 0, years: 10, returnPct, inflationPct });

  const insights = [
    {
      key: "surplus", label: "Money left each month", kind: "money", value: surplus,
      hint: surplus >= 0 ? `${fmtPct(savingsRate, 0)} of income after spending` : "You are spending more than you earn",
      tone: surplus > 0 ? "ok" : "bad", to: "savings-planner", toLabel: "See where it could go"
    },
    {
      key: "emergency", label: "Safety net", kind: "months", value: emergencyMonths,
      text: emergencyMonths >= 6 ? "6+ months covered" : `${emergencyMonths.toFixed(1)} of 6 months covered`,
      hint: emergencyGap > 0 ? `${money(emergencyGap)} short of a 6-month buffer` : "A 6-month buffer is in place",
      tone: emergencyMonths >= 6 ? "ok" : emergencyMonths >= 3 ? "warn" : "bad",
      to: "emergency-fund-planner", toLabel: "Build the safety net"
    },
    {
      key: "retirement", label: `Retirement by ${ageRet}`, kind: "text",
      text: retirement.onTrack ? "On track" : "Short of the target",
      hint: retirement.onTrack
        ? `${money(retirement.corpusNeeded)} target - you are projected to reach it`
        : `Save about ${money(retirement.requiredMonthly)}/month more to close a ${money(retirement.gap)} gap`,
      tone: retirement.onTrack ? "ok" : "warn", to: "retirement-planner", toLabel: "Open the retirement plan"
    },
    {
      key: "networth", label: "Net worth", kind: "money", value: netWorth,
      hint: `${money(investments + cash)} saved - ${money(debt)} owed`,
      tone: netWorth >= 0 ? "ok" : "warn", to: "net-worth-tracker", toLabel: "Track it over time"
    },
    {
      key: "debt", label: "Debt pressure", kind: "pct", value: debtRatio,
      text: debt > 0 ? `${fmtPct(debtRatio, 0)} of income goes to loans` : "No debt recorded",
      hint: debt > 0 ? `${money(debt)} outstanding` : "Nothing to repay",
      tone: debt === 0 ? "ok" : debtRatio <= 40 ? "warn" : "bad",
      to: "debt-planner", toLabel: "Plan the payoff"
    },
    {
      key: "future", label: "Your monthly surplus in 10 years", kind: "money", value: surplus10y.final,
      hint: surplus > 0 ? `From ${money(surplus)} invested each month at ${fmtPct(returnPct, 0)}` : "No surplus to invest yet",
      tone: surplus > 0 ? "ok" : "warn", to: "goal-planner", toLabel: "Turn it into a goal"
    }
  ];

  return {
    age, ageRet, income, expenses, cash, investments, debt, debtPayment, returnPct, inflationPct,
    surplus, savingsRate, debtRatio, netWorth, emergencyTarget, emergencyMonths, emergencyGap,
    retirement, health, surplus10y, insights
  };
}

export const TOOLS = [
  { slug: "dashboard", name: "Financial Health Dashboard", label: "How am I doing financially?", blurb: "See your savings, debt, emergency fund and progress in one place.", icon: "db", category: "decision", featured: true,
    tagline: "See your savings rate, surplus, emergency fund, goal progress and health score in one place." },
  { slug: "goal-planner", name: "Goal Planner", label: "I have a goal to reach", blurb: "Find out how much you need to invest each month to get there.", icon: "gp", category: "decision", featured: true,
    tagline: "Turn a goal into a required monthly investment - and get a plan to close the gap." },
  { slug: "retirement-planner", name: "Retirement Planner", label: "Can I retire when I want to?", blurb: "Work out your retirement number and the monthly saving to get there.", icon: "rp", category: "decision", featured: true,
    tagline: "Work out the corpus you need, the monthly saving to get there, and scenarios if you retire early or late." },
  { slug: "sip-compare", name: "SIP Strategy Lab", label: "Make my SIP better", blurb: "See how increasing your SIP or adding a yearly step-up changes your future wealth.", icon: "sc", category: "decision", featured: true,
    tagline: "Compare flat SIP, step-up SIP and different amounts or timelines on one chart." },
  { slug: "what-if-lab", name: "What-If Lab", label: "What happens if...?", blurb: "Try different returns, SIP amounts, pauses and lump sums.", icon: "wl", category: "decision", featured: true,
    tagline: "Test raise-the-SIP, step-up, lower returns, pauses and lump sums instantly." },
  { slug: "debt-planner", name: "Debt Payoff Planner", label: "When will I be debt-free?", blurb: "See your payoff timeline and what extra payments save you.", icon: "dp", category: "decision", featured: true,
    tagline: "Payoff timeline, total interest, and what extra payments save - avalanche vs snowball." },
  { slug: "emergency-fund-planner", name: "Emergency Fund Planner", label: "How big should my safety net be?", blurb: "Set your emergency fund target and the months needed to fill it.", icon: "ef", category: "decision", featured: true,
    tagline: "Set the target, see your coverage and gap, and the months needed to fill it." },
  { slug: "net-worth-tracker", name: "Net Worth Tracker", label: "What is my net worth?", blurb: "Add assets and debts, then watch the trend over time.", icon: "nw", category: "decision", featured: true,
    tagline: "Add assets and liabilities, then save dated snapshots to see the trend over time." },
  { slug: "savings-planner", name: "Savings Planner", label: "Save more each month", blurb: "Find your surplus and see what saving a little more is really worth.", icon: "sp", category: "decision", featured: true,
    tagline: "Find your monthly surplus and see what saving a little more is really worth." },
  { slug: "my-plans", name: "My Plans", label: "My saved plans", blurb: "Every plan you saved, ready to resume where you left off.", icon: "mp", category: "decision", featured: false,
    tagline: "Every plan you saved, on any device, ready to resume where you left off." }
];
...
/* Where to send the user next. Slugs may point at a calculator or a tool;
   the build/app layer resolves whichever exists. */
export const JOURNEYS = {
  "sip": [["sip-compare", "Compare SIP scenarios"], ["goal-planner", "Plan the goal behind the SIP"], ["retirement", "Check your retirement number"], ["inflation", "See what inflation does to it"]],
  "lump-sum": [["sip", "Compare with a monthly SIP"], ["sip-compare", "Model both side by side"], ["cagr", "Understand the implied return"]],
  "retirement": [["retirement-planner", "Build the full retirement plan"], ["swp", "Plan retirement withdrawals"], ["inflation", "Stress-test inflation"]],
  "swp": [["retirement-planner", "Check the corpus first"], ["sip", "Grow the corpus"], ["goal-planner", "Plan the shortfall"]],
  "fire": [["retirement-planner", "Map the full retirement plan"], ["savings-planner", "Raise your savings rate"], ["coast-fire", "See if you can coast"]],
  "coast-fire": [["fire", "Full independence number"], ["sip", "Grow the bridge corpus"]],
  "savings-goal": [["goal-planner", "Plan any goal in detail"], ["sip", "Invest the monthly amount"], ["dashboard", "See the whole picture"]],
  "emergency-fund": [["emergency-fund-planner", "Set the target and timeline"], ["dashboard", "Score your safety net"], ["savings-planner", "Find money to save"]],
  "net-worth": [["net-worth-tracker", "Track it over time"], ["dashboard", "Turn net worth into a plan"], ["savings-planner", "Improve cash flow"]],
  "debt-payoff": [["debt-planner", "Plan the payoff with extra payments"], ["dashboard", "Check debt-to-income"], ["savings-planner", "Find money to repay faster"]],
  "income-tax": [["sip", "Invest what you save in tax"], ["retirement", "Plan the long term"], ["capital-gains", "Plan the exit tax"]],
  "capital-gains": [["income-tax", "See your full tax picture"], ["sip", "Reinvest efficiently"]],
  "emi-loan": [["home-loan-prepay", "See if prepaying helps"], ["loan-compare", "Compare loan structures"], ["dashboard", "Check debt-to-income"]],
  "mortgage": [["home-loan-prepay", "Model a prepayment"], ["loan-compare", "Compare tenures"]],
  "home-loan-prepay": [["emi-loan", "Back to the base EMI"], ["loan-compare", "Compare options"]],
  "inflation": [["goal-planner", "Set an inflation-aware goal"], ["sip", "Grow real, not just nominal"]],
  "cagr": [["sip", "Apply it to a monthly plan"], ["sip-compare", "Benchmark scenarios"]],
  "compound-interest": [["sip", "Compare with monthly investing"], ["goal-planner", "Plan a goal"]],
  "fd": [["savings-planner", "Build the deposit from cash flow"], ["inflation", "Check the real return"]],
  "ppf": [["retirement", "Long-term tax-free bucket"], ["inflation", "Real value check"]],
  "rd": [["sip", "Compare with a SIP"], ["savings-goal", "Plan a goal"]],
  "position-size": [["risk-reward", "Check the payoff"], ["profit-loss", "Model the outcome"]],
  "risk-reward": [["position-size", "Size the trade"], ["profit-loss", "See the P&L"]],
  "profit-loss": [["risk-reward", "Judge the payoff"], ["position-size", "Size the next trade"]],
  "xirr": [["sip", "Project the same cash flows"], ["cagr", "Compare with a simple CAGR"]],
  "dashboard": [["savings-planner", "Improve your cash flow"], ["goal-planner", "Plan a specific goal"], ["sip", "Start or raise a SIP"]],
  "goal-planner": [["sip", "Model the monthly contribution"], ["what-if-lab", "Test the trade-offs"], ["inflation", "See inflation's impact"]],
  "sip-compare": [["goal-planner", "Turn the winner into a goal plan"], ["what-if-lab", "Stress-test the assumptions"], ["sip", "Open a single plan"]],
  "what-if-lab": [["goal-planner", "Build the plan you tested"], ["sip-compare", "Compare alternatives"], ["retirement-planner", "Apply it to retirement"]],
  "savings-planner": [["dashboard", "Score your whole position"], ["emergency-fund-planner", "Size the safety net"], ["sip", "Invest the surplus"]],
  "retirement-planner": [["goal-planner", "Plan other goals too"], ["sip", "Model the monthly SIP"], ["what-if-lab", "Stress-test the plan"]],
  "debt-planner": [["savings-planner", "Find money for extra payments"], ["dashboard", "Check debt-to-income"], ["emergency-fund-planner", "Protect the buffer"]],
  "emergency-fund-planner": [["savings-planner", "Find the monthly saving"], ["debt-planner", "Balance debt vs buffer"], ["dashboard", "Score your safety net"]],
  "net-worth-tracker": [["dashboard", "Score your whole position"], ["my-plans", "See every saved plan"], ["goal-planner", "Turn net worth into a goal"]],
  "my-plans": [["dashboard", "Start from your position"], ["goal-planner", "Plan a goal"], ["net-worth-tracker", "Track your progress"]],
  "gst-tax": [["income-tax", "See your full income-tax picture"], ["expense-ratio", "Read tax as a cost ratio"], ["sip", "Invest what the structure saves"]],
  "investment-return": [["cagr", "Annualise the return"], ["xirr", "Handle multiple cash flows"], ["real-return", "Strip out inflation"]],
  "real-return": [["inflation", "Understand the inflation input"], ["cagr", "Compare nominal returns"], ["investment-return", "Model your own return"]],
  "expense-ratio": [["savings-planner", "See the cash-flow impact"], ["dashboard", "Score your whole position"], ["investment-return", "What fees do to returns"]],
  "stp": [["swp", "Plan the withdrawal phase"], ["sip", "Build the corpus you transfer"], ["lump-sum", "Compare a one-time investment"]],
  "loan-compare": [["emi-loan", "Model a single loan"], ["mortgage", "Run the home-loan numbers"], ["home-loan-prepay", "See if prepaying helps"]],
  "credit-card-interest": [["debt-planner", "Plan the payoff"], ["savings-planner", "Find money to repay"], ["dashboard", "Check debt-to-income"]]
};

export function nextSteps(slug) {
  return (JOURNEYS[slug] || []).map(([target, why]) => {
    const calc = CALCULATORS[target];
    const tool = getTool(target);
    const meta = calc ? { slug: calc.slug, name: calc.name, kind: "calc" } : tool ? { slug: tool.slug, name: tool.name, kind: "tool" } : null;
    return meta ? { ...meta, why } : null;
  }).filter(Boolean);
}

/* =========================================================================
   "Ask FinHub" - map a plain-language question to the right destination
   ========================================================================= */
const ASK_STOP = new Set(("a an and are as at be but by can could do does for from get got had has have how i if in into is it its me much many my need of on or should so that the their them then there these this to too us want was we were what when where which who will with would you your make makes best good way ways start help reach growing become").split(" "));
/* Extra plain-language synonyms per slug; unknown slugs are ignored. */
const ASK_HINTS = {
  "sip": "sip monthly investment mutual fund systematic invest every month invest monthly",
  "sip-compare": "compare sip step up step-up increase sip better sip strategy",
  "lump-sum": "lump sum one time one-time lump",
  "compound-interest": "compound interest compounding",
  "cagr": "cagr annual growth growth rate annualised annualized",
  "investment-return": "roi return on investment absolute return profit",
  "real-return": "real return after inflation adjusted",
  "expense-ratio": "expense ratio fund fees charges",
  "xirr": "xirr irr multiple cashflows cash flow",
  "stp": "stp systematic transfer",
  "swp": "swp systematic withdrawal drawdown pension",
  "fire": "fire financial independence retire early early retirement",
  "coast-fire": "coast fire coast",
  "emi-loan": "emi loan home loan car loan personal loan borrow installment equated monthly",
  "mortgage": "mortgage housing property home",
  "home-loan-prepay": "prepay prepayment part payment foreclosure",
  "loan-compare": "compare loans loan comparison interest rate compare",
  "debt-payoff": "debt payoff snowball avalanche clear debt debt free",
  "credit-card-interest": "credit card card interest revolving",
  "savings-goal": "savings goal save for goal target",
  "fd": "fd fixed deposit fixed",
  "ppf": "ppf public provident fund",
  "rd": "rd recurring deposit recurring",
  "emergency-fund": "emergency fund rainy day safety",
  "income-tax": "income tax tax itr slab salary take home",
  "gst-tax": "gst goods services tax vat",
  "inflation": "inflation cost of living future cost rising prices purchasing power",
  "capital-gains": "capital gain ltcg stcg equity tax stocks tax",
  "position-size": "position size how many shares risk per trade trade size",
  "risk-reward": "risk reward ratio payoff",
  "profit-loss": "profit loss pnl p and l trade",
  "net-worth": "net worth assets liabilities wealth",
  "retirement": "retirement retire pension corpus old age retire at",
  "dashboard": "how am i doing financial health overview position savings rate surplus",
  "goal-planner": "goal planner target save for education house car child down payment",
  "retirement-planner": "retirement planner when can i retire retire corpus pension",
  "what-if-lab": "what if scenario pause sensitivity test assumptions",
  "debt-planner": "debt planner when will i be debt free payoff plan debt",
  "emergency-fund-planner": "emergency fund planner safety net months buffer",
  "net-worth-tracker": "net worth tracker track assets liabilities trend",
  "savings-planner": "save more saving rate increase savings surplus cash flow",
  "my-plans": "saved plans my plans resume"
};

let ASK_INDEX = null;
/* Whole-phrase intents worth a strong boost (checked against the raw query). */
const ASK_PHRASES = {
  "retirement-planner": ["when can i retire", "can i retire", "when should i retire", "retirement plan"],
  "dashboard": ["how am i doing"],
  "debt-planner": ["debt free", "pay off debt", "clear debt", "pay off my loan"],
  "emergency-fund-planner": ["emergency fund", "safety net"],
  "sip-compare": ["make my sip better", "increase my sip"],
  "what-if-lab": ["what if"],
  "goal-planner": ["reach my goal", "save for a goal"],
  "net-worth-tracker": ["net worth tracker", "track my net worth"],
  "fire": ["retire early", "financial independence"]
};
function buildAskIndex() {
  const idx = [];
  const add = (type, slug, title, priority, parts) => {
    const text = parts.filter(Boolean).join(" ").toLowerCase();
    idx.push({ type, slug, title, priority, text, tokens: new Set(text.split(/[^a-z0-9]+/).filter((t) => t.length > 1)) });
  };
  CALC_LIST.forEach((c) => add("calc", c.slug, c.name, 2, [
    c.slug.replace(/-/g, " "), c.name, CATEGORIES[c.category], (c.fields || []).map((f) => f.label).join(" "), ASK_HINTS[c.slug]
  ]));
  TOOLS.forEach((t) => {
    if (t.slug === "my-plans") return;
    add("tool", t.slug, t.label || t.name, 1, [t.slug.replace(/-/g, " "), t.name, t.label, t.blurb, t.tagline, ASK_HINTS[t.slug]]);
  });
  return idx;
}

/* Rank destinations for a free-text query. Returns [{type,slug,title,href}]. */
export function askIntent(query, limit = 4) {
  const q = String(query == null ? "" : query).toLowerCase().replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
  if (!q) return [];
  const tokens = q.split(" ").filter((t) => t.length > 1 && !ASK_STOP.has(t));
  if (!tokens.length) return [];
  if (!ASK_INDEX) ASK_INDEX = buildAskIndex();
  const bigrams = [];
  for (let i = 0; i < tokens.length - 1; i++) bigrams.push(`${tokens[i]} ${tokens[i + 1]}`);
  const scored = [];
  ASK_INDEX.forEach((d) => {
    let score = 0;
    tokens.forEach((t) => { if (d.tokens.has(t)) score += 2; else if (d.text.includes(t)) score += 1; });
    bigrams.forEach((b) => { if (d.text.includes(b)) score += 2; });
    (ASK_PHRASES[d.slug] || []).forEach((p) => { if (q.includes(p)) score += 4; });
    if (score > 0) scored.push({ type: d.type, slug: d.slug, title: d.title, score, priority: d.priority });
  });
  scored.sort((a, b) => b.score - a.score || b.priority - a.priority || a.title.length - b.title.length);
  return scored.slice(0, limit).map(({ type, slug, title, score }) => ({ type, slug, title, score, href: `/${slug}/` }));
}

/* =========================================================================
   Carry the homepage snapshot numbers into the planners.
   Pure: reads a profile object, returns the subset of planner state keys
   that should be pre-filled. Missing/degenerate profiles return null.
   ========================================================================= */
export const PROFILE_KEY = "finhub_profile_v1";
export function normalizeProfile(input) {
  const p = input || {};
  const num = (v, d) => { if (v === "" || v == null) return d; const n = Number(v); return Number.isFinite(n) && n >= 0 ? n : d; };
  return {
    age: num(p.age, 30),
    income: num(p.income, 0),
    expenses: num(p.expenses, 0),
    cash: num(p.cash, 0),
    investments: num(p.investments, 0),
    debt: num(p.debt, 0),
    debtPayment: num(p.debtPayment, 0),
    returnPct: num(p.returnPct, 12),
    inflationPct: num(p.inflationPct, 6)
  };
}

export function profileToState(slug, input) {
  const p = normalizeProfile(input);
  const empty = !p.income && !p.expenses && !p.cash && !p.investments && !p.debt && !p.debtPayment;
  if (empty) return null;
  const surplus = Math.max(0, p.income - p.expenses - p.debtPayment);
  const fixed = Math.round((p.expenses * 0.6) / 1000) * 1000;
  const variable = Math.max(0, p.expenses - fixed);
  const maps = {
    dashboard: { income: p.income, fixed, variable, debt: p.debtPayment, monthlyInvestment: surplus, savings: p.cash + p.investments, returnPct: p.returnPct, inflationPct: p.inflationPct },
    "savings-planner": { income: p.income, fixed, variable, debt: p.debtPayment, monthlyInvestment: surplus, savings: p.cash, returnPct: p.returnPct, inflationPct: p.inflationPct },
    "emergency-fund-planner": { essentialMonthly: p.expenses, current: p.cash, monthly: surplus },
    "retirement-planner": { ageNow: p.age, monthlyExpense: p.expenses, corpusNow: p.investments, monthlySave: surplus, returnPct: p.returnPct, inflationPct: p.inflationPct },
    "debt-planner": { balance1: p.debt, payment1: p.debtPayment },
    "goal-planner": { monthly: surplus, currentSavings: p.investments, returnPct: p.returnPct, inflationPct: p.inflationPct },
    "what-if-lab": { monthly: surplus, initial: p.investments, returnPct: p.returnPct, inflationPct: p.inflationPct }
  };
  const m = maps[slug];
  if (!m) return null;
  const out = {};
  Object.keys(m).forEach((k) => { const n = Number(m[k]); if (Number.isFinite(n) && n > 0) out[k] = n; });
