...
const TOTAL_COUNT = CALC_LIST.length;
const HERO_SUB = HOME.heroSub.replace("Accurate finance calculators", `${TOTAL_COUNT} accurate finance calculators`);

  let out = "";
  out += `<div class="container hero">`;
  out += `<span class="eyebrow">${esc(HOME.popularNote)}</span>`;
  out += `<h1>${esc(HOME.heroTitle)}</h1>`;
  out += `<p>${esc(HERO_SUB)}</p>`;
  out += `</div>`;
